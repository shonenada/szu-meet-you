...
class Teach(BuildingHandler):
    ...
    def get(self):
        ...
        number = user['number']
        course = special.TeachingBuilding.get_class_infor_by_truename_and_number(truename, number)
        classes = []
        for x in course:
            classes.append(special.TeachingBuilding.get_course_infor_by_classid(x['cid']))
...
class ClassMate(szumu.web.Controller):
    ...
    def get(self, classid):
        ...
        classinfor = []
        for x in classes:
            if UserModel.check_truename(x['truename'], 
                                        username) and UserModel.check_number(x['number'], 
                                                                             username):
                mate = UserModel.get_user_by_truename_and_number(x['truename'], x['number'])
